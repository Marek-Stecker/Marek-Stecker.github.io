# Unity Developer Portfolio

Hi, I'm **Marek Stecker**,
aspiring Unity developer focused on crafting innovative games.

---

## Experience
- Unity Engine - 7 years
- C# Programming - 3 years
- Others - Arduino programming, video editing, frontend+backend web development, HLSL shaders

---

## Projects

### Training App for Climbers
A new way how climbers can train with unlimited possibilities. This app connects to a climbing wall with LEDs and allows you to control it - create thousands of routes, edit them and create a customized training program. Uses BLE to connect to ESP32, that controls the LEDs on the wall.
- Available for: Android, iOS
- Built with: Unity, C#, Arduino IDE
- [Google Play](https://play.google.com/store/apps/details?id=com.Matrettr.LEDWall&hl)
- [App Store](https://apps.apple.com/us/app/mywaywall/id6444047230)

## Horror Game - Light Bearer
A 2d horror game with unique light mechanics to be released on Steam. You are the Light bearer in an abondoned lighthouse and need to use your magical tool to transfer light into light sources. Keep the environment lit, to defend yourself from the shadowy creatures.
- [Steam](https://store.steampowered.com/app/3959360/Light_Bearer/)
- 
---

## Tutoring

### YouTube Channel
I spent over 2 years making long-form educational content on YouTube. 
- 180+ Unity and C# tutorials
- 6500+ subscibers
- [YouTube channel](https://www.youtube.com/@freedomcoding/videos)

### One-On-One Tutoring
For the past few years I have been individually tutoring Unity and C# programming online to people all over the world. 
- 20+ students
- 300+ hours of lessons
- [Superprof account](https://www.superprof.com/will-help-you-with-unity-game-development-and-coding-passionate-and-friendly-tutor-who-will-teach-you-only-the-best.html)

---

## Game Jams

---
